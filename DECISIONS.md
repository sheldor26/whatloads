...
**Decision.** The name is whatloads — what actually loads, and what does not —
the vocabulary is agent-neutral, and AGENTS.md is a first-class input rather
than a compatibility note. Claude Code specifics live in the checks, which name
it plainly, not in the branding. The first choice, agentdoctor, was rejected by
npm as too close to an existing package; see M-0001.
