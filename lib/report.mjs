...
export function printUserScopeCost(facts, projectDirs) {
  ...
  if (facts.agents.length) {
    console.log(`  ${facts.agents.length} subagent description${facts.agents.length === 1 ? '' : 's'} under ~/.claude/agents/, loaded at launch the same way:`);
    for (const a of [...facts.agents].sort((a, b) => b.chars - a.chars)) {
      console.log(`    ${a.name}  ${a.chars} chars`);
    }
    console.log('');
  }
